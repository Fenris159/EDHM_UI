  ...
  <div class="data-table table-responsive" ref="dataTable" v-if="groupedElements.length > 0">
    <div v-for="(group, groupIndex) in groupedElements" :key="group.category">
      ...
      <ul v-if="showContextMenuFlag" :style="contextMenuStyle" class="dropdown-menu show" ref="contextMenu">
        <li><a class="dropdown-item" href="#" @click="onContextMenu_Click('RemoveUserSettings')">Remove from User Settings..</a> </li>        
      </ul>
...
export default {
    ...
    saveChanges: function () {
      if (!this.saveChangesDebounced) {
        this.saveChangesDebounced = this.debounce(async function () {
          try {
            await window.api.saveUserSettings(JSON.parse(JSON.stringify(this.dataSource)));
            this.$emit('OnUserSettings_Saved', JSON.parse(JSON.stringify(this.dataSource))); //<- Event listen on 'NavBars.vue'
  ...
  mounted() {
    eventBus.on('DoLoadUserSettings', this.OnInitialize);
    eventBus.on('FindKeyInUserSettings', this.DoFindAndSelectRow);
  },
  beforeUnmount() {
